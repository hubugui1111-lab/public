...
void print_phase(const char*name,int party,const Phase&p){
  std::cout<<"EXP215_PHASE party="<<party<<" name="<<name
...
int main(int argc,char**argv){
  ...
  auto bool_triples=prepare_bool_triples(io,ot,party,4*N);
  ...
  auto ph_cert=measure(io,[&]{
    ...
    auto layer_out=beaver_bool_and(io,party,layer_x,layer_y,
    ...
    for(int i=0;i<N;++i){
      cert.data[i]=neg_cert.data[i]^layer_out.data[i];
      safe_positive.data[i]=layer_out.data[N+i];
    }
  });
  ...
  std::cout<<"EXP215_RESULT party="<<party<<" N="<<N<<" capacity="<<CAP
