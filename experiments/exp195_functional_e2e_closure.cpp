...
void print_phase(const char* name,int party,const Phase& p){
  std::cout<<"EXP195_PHASE party="<<party<<" name="<<name
...
int main(int argc,char**argv){
  ...
  FixArray predz;
  auto ph_predout=measure(io,[&]{ predz=math.fix->mul(pred_pos_arith,z,ELL); });
  ...
  // F. Functional end-to-end closure: merge certified prediction with the
  // native exact fallback under the secret certificate bit.  This first
  // closure deliberately computes the fallback densely for all N coordinates;
  // it proves there is no plaintext shortcut before sparse-tail optimization.
  FixArray final_out;
  auto ph_merge=measure(io,[&]{
    final_out=math.fix->if_else(cert,predz,baseline);
  });
  ...
  for(int i=0;i<N;++i){
    ...
    if(party==ALICE){
      ...
      mismatches += ((pub_predz.data[i]&MASK)!=(exp_pz&MASK));
      const uint64_t exp_relu=zc>0?z_clear[i]:0;
      mismatches += ((pub_full.data[i]&MASK)!=(exp_relu&MASK));
      mismatches += ((pub_final.data[i]&MASK)!=(exp_relu&MASK));
    }
  }
  ...
  print_phase("full_relu_fallback",party,ph_full);
  print_phase("secret_merge",party,ph_merge);

  const uint64_t pre_tail_bytes=ph_probe.bytes+ph_route.bytes+ph_leaf.bytes+
      ph_arith.bytes+ph_cert.bytes+ph_predout.bytes;
  const uint64_t pre_tail_rounds=ph_probe.rounds+ph_route.rounds+ph_leaf.rounds+
      ph_arith.rounds+ph_cert.rounds+ph_predout.rounds;
  const double pre_tail_ms=ph_probe.ms+ph_route.ms+ph_leaf.ms+
      ph_arith.ms+ph_cert.ms+ph_predout.ms;
  const uint64_t e2e_bytes=pre_tail_bytes+ph_full.bytes+ph_merge.bytes;
  const uint64_t e2e_rounds=pre_tail_rounds+ph_full.rounds+ph_merge.rounds;
  const double e2e_ms=pre_tail_ms+ph_full.ms+ph_merge.ms;

  std::cout<<"EXP195_RESULT party="<<party
           <<" N="<<N<<" internal_probes="<<INTERNAL<<" leaves="<<LEAVES
           <<" target_leaf="<<TARGET_LEAF
           <<" cert_count="<<cert_count
           <<" cert_fraction="<<(double(cert_count)/N)
           <<" expected_cert="<<expected_cert
           <<" pre_tail_bytes="<<pre_tail_bytes
           <<" pre_tail_rounds="<<pre_tail_rounds
           <<" pre_tail_ms="<<pre_tail_ms
           <<" fallback_full_relu_bytes="<<ph_full.bytes
           <<" fallback_full_relu_rounds="<<ph_full.rounds
           <<" fallback_full_relu_ms="<<ph_full.ms
           <<" merge_bytes="<<ph_merge.bytes
           <<" merge_rounds="<<ph_merge.rounds
           <<" merge_ms="<<ph_merge.ms
           <<" e2e_bytes="<<e2e_bytes
           <<" e2e_rounds="<<e2e_rounds
           <<" e2e_ms="<<e2e_ms
           <<" baseline_full_relu_bytes="<<ph_full.bytes
           <<" baseline_full_relu_rounds="<<ph_full.rounds
           <<" baseline_full_relu_ms="<<ph_full.ms
