...
void print_phase(const char*name,int party,const Phase&p){
  std::cout<<"EXP221_PHASE party="<<party<<" name="<<name
...
int main(int argc,char**argv){
  ...
  if(party==ALICE){
    ...
    for(int leaf=0;leaf<LEAVES;++leaf)for(int i=0;i<N;++i){
      clear_bounds[size_t(leaf)*N+i]=BOUND;
      clear_signs[size_t(leaf)*N+i]=pred_bit(leaf,i);
    }
  }
  ...
  auto k_route=prepare_kcompare(math,io,party,rng,LEAVES);
  ...
  BoolArray leaf_onehot(party,LEAVES);
  auto ph_route=measure(io,[&]{
    auto probe_a=math.fix->B2A(probe_sign,false,ELL);
    std::vector<uint64_t> score_d(LEAVES,0);
    for(int leaf=0;leaf<LEAVES;++leaf){
      uint64_t score=0;
      for(int depth=0;depth<DEPTH;++depth){
        const int node=path_internal_node(leaf,depth);
        const uint8_t branch=target_branch(leaf,depth);
        if(branch) score=(score+probe_a.data[node])&MASK;
        else score=(score+((party==ALICE?1ULL:0ULL)-probe_a.data[node]))&MASK;
      }
      score_d[leaf]=(score-(party==ALICE?uint64_t(DEPTH):0ULL))&MASK;
    }
    leaf_onehot=kangaroo_ge0(io,party,rng,k_route,score_d);
  });
  ...
  int opened_leaf=-1,onehot_weight=0;
  for(int l=0;l<LEAVES;++l){onehot_weight+=pub_leaf.data[l];if(pub_leaf.data[l])opened_leaf=l;}
  mismatches+=int(onehot_weight!=1)+int(opened_leaf!=TARGET_LEAF);
  if(accepted)mismatches+=(opened_weight!=CAP);

  print_phase("probe15",party,ph_probe);print_phase("kangaroo_pathscore_route",party,ph_route);
  ...
  print_phase("kcompare_probe_preprocess",party,k_probe.phase);print_phase("kcompare_route_preprocess",party,k_route.phase);
  ...
  std::cout<<"EXP221_RESULT party="<<party<<" N="<<N<<" capacity="<<CAP
