...
struct PreparedRouteGC222{
  ...
  struct CountDummyResult{
    bool accepted=false;
    std::vector<uint8_t> opened_delta;
  };

  CountDummyResult run_count_dummy(uint64_t w_share,const BoolArray&mask_share){
    if(mask_share.size!=CAP)throw std::runtime_error("count dummy mask size");
    constexpr int WBITS=12; // N=3072 < 2^12; low bits of additive shares suffice modulo 2^12.
    Integer wa(WBITS,math.party==ALICE?int64_t(w_share&((1u<<WBITS)-1)):0,ALICE);
    Integer wb(WBITS,math.party==BOB?int64_t(w_share&((1u<<WBITS)-1)):0,BOB);

    // Declare all private mask inputs before evaluating the circuit so the
    // already-refilled COT buffer serves this whole stage without a refill.
    std::vector<Bit> ma,mb;ma.reserve(CAP);mb.reserve(CAP);
    for(int j=0;j<CAP;++j){
      ma.emplace_back(math.party==ALICE?bool(mask_share.data[j]&1):false,ALICE);
      mb.emplace_back(math.party==BOB?bool(mask_share.data[j]&1):false,BOB);
    }
...
FixArray dabit_b2a_opened_delta(int party,const std::vector<uint8_t>&delta,const DaBitPack&d){
  const int n=int(delta.size());
  if(d.a_bool.size!=n||d.a_add.size!=n)throw std::runtime_error("opened delta size");
  FixArray y(party,n,false,ELL,0);
  for(int i=0;i<n;++i){
    const uint8_t opened=delta[i]&1;
    y.data[i]=opened
      ? (((party==ALICE)?1ULL:0ULL)-d.a_add.data[i])&MASK
      : d.a_add.data[i]&MASK;
  }
...
void print_phase(const char*name,int party,const Phase&p){
  std::cout<<"EXP224_PHASE party="<<party<<" name="<<name
...
struct AdditivePrepFile{
  uint32_t M=0;
  std::vector<uint32_t> own_perm,own_delta,recvA,recvB;
};
AdditivePrepFile read_additive_prep(const std::string&path){
  std::ifstream is(path,std::ios::binary);
  if(!is)throw std::runtime_error("open additive prep");
  char magic[8];is.read(magic,8);
  if(std::string(magic,8)!="E224ADD1")throw std::runtime_error("additive prep magic");
  AdditivePrepFile x;is.read((char*)&x.M,4);
  if(x.M!=M)throw std::runtime_error("additive prep M");
  x.own_perm.resize(M);
  x.own_delta.resize(size_t(M)*3);
  x.recvA.resize(size_t(M)*3);
  x.recvB.resize(size_t(M)*3);
  is.read((char*)x.own_perm.data(),x.own_perm.size()*4);
  is.read((char*)x.own_delta.data(),x.own_delta.size()*4);
  is.read((char*)x.recvA.data(),x.recvA.size()*4);
  is.read((char*)x.recvB.data(),x.recvB.size()*4);
  if(!is)throw std::runtime_error("additive prep truncated");
  return x;
}
void install_additive_prep(int party,const AdditivePrepFile&x,PreparedPerm&p0,PreparedPerm&p1){
  PreparedPerm& own = party==ALICE ? p0 : p1;
...
int main(int argc,char**argv){
  int party=0,port=32000;std::string address="127.0.0.1",prep_path;
  ArgMapping amap;amap.arg("r",party,"role");amap.arg("p",port,"port");amap.arg("ip",address,"ip");amap.arg("prep",prep_path,"prep");amap.parse(argc,argv);
  if((party!=ALICE&&party!=BOB)||prep_path.empty())return 2;
  ...
  // Exp224: fast permutation correlations were generated externally by the
  // maintained Secure-Join PCG and converted offline from XOR to additive
  // shares.  No BFV arbitrary-permutation preprocessing runs here.
  PreparedPerm p0,p1;
  const auto load_t0=Clock::now();
  auto additive_prep=read_additive_prep(prep_path);
  install_additive_prep(party,additive_prep,p0,p1);
  const double shuffle_material_load_ms=
    std::chrono::duration<double,std::milli>(Clock::now()-load_t0).count();
  ...
  // Exp223: one prepared GC reconstructs the small secret count, reveals
  // only the public accept bit and daBit-masked dummy bits, then conversion to
  // additive dummy shares is entirely local.
  PreparedRouteGC222::CountDummyResult count_dummy;
  auto ph_dummy=measure(io,[&]{
    count_dummy=prepared_route->run_count_dummy(w_share,dummy_dabit.a_bool);
  });
  const bool accepted=count_dummy.accepted;
  ...
  if(accepted){
    FixArray dummy_a;
    ph_flags=measure(io,[&]{dummy_a=dabit_b2a_opened_delta(party,count_dummy.opened_delta,dummy_dabit);});
  ...
  print_phase("dual_share_local_count",party,ph_count);print_phase("prepared_gc_accept_dummy",party,ph_dummy);
  print_phase("local_dabit_dummy_convert",party,ph_flags);print_phase("shuffle_forward_plus_open",party,ph_forward);
  ...
  print_phase("bool_triple_preprocess",party,bool_triples.prep);print_phase("dummy_dabit_preprocess",party,dummy_dabit.prep);print_phase("mixed_merge_preprocess",party,merge_triple.prep);print_phase("route_gc_preprocess",party,ph_route_pre);
  print_phase("shuffle_setup_removed",party,ph_shuffle_setup);print_phase("shuffle_preprocess_external",party,ph_shuffle_pre);

  std::cout<<"EXP224_RESULT party="<<party<<" N="<<N<<" capacity="<<CAP
           ...
           <<" shuffle_preprocess_bytes=0 shuffle_preprocess_ms=0"
           <<" shuffle_material_load_ms="<<shuffle_material_load_ms
