...
class PreparedSharedLeafLookup16 {
  FPMath& math_;
  sci::PRG128& rng_;
  int n_,ell_;
  uint64_t mask_;
 ...
 public:
  ...
  PreparedSharedLeafLookup16(FPMath& math,sci::PRG128& rng,int n,int ell,
      const std::vector<uint64_t>& clear_bounds,
      const std::vector<uint8_t>& clear_signs)
      :math_(math),rng_(rng),n_(n),ell_(ell),
       mask_((uint64_t(1)<<ell)-1),sign_bytes_((size_t(n)+7)/8),
       payload_bytes_(size_t(3)*n+sign_bytes_) {
    require(n_>0 && ell_==24);
    ...
    if(math_.party==sci::ALICE){
      ...
      for(int row=0;row<16;++row){
        ...
        for(int i=0;i<n_;++i){
          ...
          dst[3*size_t(i)+0]=uint8_t(v);
          dst[3*size_t(i)+1]=uint8_t(v>>8);
          dst[3*size_t(i)+2]=uint8_t(v>>16);
        }
        auto* sb=dst+size_t(3)*n_;
        for(int i=0;i<n_;++i){
  ...
  LeafLookupResult query(uint8_t index_share){
    ...
    if(math_.party==sci::ALICE){
      ...
      for(int i=0;i<n_;++i){
        out.bound_shares[i]=uint64_t(clear[3*size_t(i)])|
          (uint64_t(clear[3*size_t(i)+1])<<8)|
          (uint64_t(clear[3*size_t(i)+2])<<16);
      }
      out.sign_shares.resize(n_);
      const auto* sb=clear.data()+size_t(3)*n_;
