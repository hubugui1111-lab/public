...
int main(int argc,char**argv){
  ...
  // Evaluate both strict bounds in one batched comparison call:
  // first half: x < upper(=2a), second half: 0 < x.
  // The frozen fixed-point range keeps all operands/differences inside the
  // signed ELL-bit domain, so there is no subtraction overflow.
  FixArray cmp_l(party,2*N,true,ELL,0),cmp_r(party,2*N,true,ELL,0);
  for(int i=0;i<N;++i){
    cmp_l.data[i]=aligned.data[i]; cmp_r.data[i]=upper.data[i];
    cmp_l.data[N+i]=0; cmp_r.data[N+i]=aligned.data[i];
  }
  BoolArray cmp,lo(party,N),hi(party,N),cert;
  auto ph_cert=measure(io,[&]{
    cmp=math.fix->LT(cmp_l,cmp_r);
    for(int i=0;i<N;++i){hi.data[i]=cmp.data[i];lo.data[i]=cmp.data[N+i];}
    cert=math.bool_op->AND(hi,lo);
  });
  ...
  print_phase("certificate_interval_compare",party,ph_cert);
