...
int main(int argc,char**argv){
  ...
  FixArray beta(party,N,true,ELL,0);
  ...
  auto ph_leaf=measure(io,[&]{
    ...
    if(party==ALICE){
      ...
      for(int i=0;i<N;++i){
        beta.data[i]=random_words[i]&MASK;
        ...
        for(int c=0;c<LEAVES;++c){
          ...
          const uint64_t clear_beta=enc_signed(pos?-BETA_MAG:BETA_MAG,ELL);
          const uint64_t beta_b=(clear_beta-beta.data[i])&MASK;
          const uint64_t pred_b=uint64_t(uint8_t(pos)^pred_a[i]);
          messages[i][c]=beta_b|(pred_b<<ELL);
        }
  ...
  print_phase("leaf_ot_lookup",party,ph_leaf); print_phase("certificate_arithmetic",party,ph_arith);
