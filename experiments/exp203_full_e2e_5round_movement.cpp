...
void print_phase(const char*name,int party,const Phase&p){
  std::cout<<"EXP203_PHASE party="<<party<<" name="<<name
...
int main(int argc,char**argv){
  ...
  for(int i=0;i<N;++i){lhs.data[i]=0;rhs.data[i]=aligned.data[i];lhs.data[N+i]=aligned.data[i];rhs.data[N+i]=bound.data[i];}
  BoolArray cert;auto ph_cert=measure(io,[&]{
    auto cmp=math.fix->LT(lhs,rhs);BoolArray lo(party,N),hi(party,N);
  ...
  Prefix counts;bool accepted=false;
  auto ph_count=measure(io,[&]{counts=prefix(math,marks);accepted=shared_residual::accepted(math,counts,CAP);});

  FixArray final_out(party,N,true,ELL,0);
  Phase ph_dummy{},ph_flags{},ph_forward{},ph_tail{},ph_inverse{},ph_merge{},ph_overflow{};
  int opened_weight=0;

  if(accepted){
    BoolArray dummy(party,CAP);
    ph_dummy=measure(io,[&]{
      uint64_t need=((party==ALICE?uint64_t(CAP):0)-counts.sums.back())&511ULL;
      dummy=make_dummy_flags(math,need);
    });

    BoolArray flags(party,M);
    for(int i=0;i<N;++i)flags.data[i]=marks.data[i];
    for(int j=0;j<CAP;++j)flags.data[N+j]=dummy.data[j];
    FixArray flags_a;
    ph_flags=measure(io,[&]{flags_a=math.fix->B2A(flags,false,ELL);});

    std::vector<uint32_t> local(size_t(M)*2,0),shuffled;
    for(int i=0;i<N;++i){
      local[size_t(i)*2]=uint32_t(flags_a.data[i])&RMASK;
      local[size_t(i)*2+1]=uint32_t(z.data[i])&RMASK;
    }
    for(int j=0;j<CAP;++j)local[size_t(N+j)*2]=uint32_t(flags_a.data[N+j])&RMASK;
  ...
  print_phase("secret_count_accept",party,ph_count);print_phase("secret_dummy_padding",party,ph_dummy);
  print_phase("flags_B2A",party,ph_flags);print_phase("shuffle_forward_plus_open",party,ph_forward);
  ...
  std::cout<<"EXP203_RESULT party="<<party<<" N="<<N<<" capacity="<<CAP
