...
BoolTriplePack prepare_bool_triples(IOPack&io,OTPack&ot,int party,int n){
  BoolTriplePack out;
...
FixArray prepared_b2a_fast(IOPack&io,OTPack&ot,PRG128&rng,int party,const BoolArray&x){
  const int n=x.size;
...
FixArray prepared_mux_fast(IOPack&io,OTPack&ot,PRG128&rng,int party,
...
#pragma omp parallel num_threads(2)
  {
    if(omp_get_thread_num()==0){
      if(party==ALICE)ot.iknp_straight->send(ptrs.data(),n,ELL);
      else ot.iknp_straight->recv(recv_val.data(),sel.data,n,ELL);
    }else{
      if(party==BOB)ot.iknp_reversed->send(ptrs.data(),n,ELL);
      else ot.iknp_reversed->recv(recv_val.data(),sel.data,n,ELL);
    }
  }
  FixArray y(party,n,true,ELL,0);
  for(int i=0;i<n;++i)
    y.data[i]=(x.data[i]*uint64_t(sel.data[i]&1)+recv_val[i]-send_mask[i])&MASK;
  return y;
}
...
void print_phase(const char*name,int party,const Phase&p){
  std::cout<<"EXP216_PHASE party="<<party<<" name="<<name
...
int main(int argc,char**argv){
  ...
  auto bool_triples=prepare_bool_triples(io,ot,party,2*N);

  // Offline OT tapes for two B2A calls (N + CAP) plus one N-value merge mux.
  ot.iknp_straight->set_precomp_batch_size(2*N+CAP);
  ot.iknp_reversed->set_precomp_batch_size(N);
  Phase ph_fast_ot_pre=measure(io,[&]{
#pragma omp parallel num_threads(2)
    {
  ...
  auto ph_cert=measure(io,[&]{
    ...
    cert=beaver_bool_and(io,party,lo,hi,
  ...
  auto ph_count=measure(io,[&]{
    marks_a=prepared_b2a_fast(io,ot,rng,party,marks);
  ...
  if(accepted){
    FixArray dummy_a;
    ph_flags=measure(io,[&]{dummy_a=prepared_b2a_fast(io,ot,rng,party,dummy);});
    ...
    ph_merge=measure(io,[&]{
      auto known=prepared_mux_fast(io,ot,rng,party,safe_positive,z);
  ...
  print_phase("kcompare_count_preprocess",party,k_count.phase);print_phase("bool_triple_preprocess",party,bool_triples.prep);print_phase("fast_ot_preprocess",party,ph_fast_ot_pre);
  print_phase("shuffle_setup",party,ph_shuffle_setup);print_phase("shuffle_preprocess",party,ph_shuffle_pre);

  std::cout<<"EXP216_RESULT party="<<party<<" N="<<N<<" capacity="<<CAP
