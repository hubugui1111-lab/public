...
BoolTriplePack prepare_bool_triples(FPMath&math,IOPack&io,OTPack&ot,int party,int n){
  BoolTriplePack out;
  ...
  out.prep=measure(io,[&]{
    ...
    BoolArray packed(party,3*first);
    for(int i=0;i<first;++i){
      packed.data[i]=out.a[i];
      packed.data[first+i]=out.b[i];
      packed.data[2*first+i]=out.c[i];
    }
    auto add=math.fix->B2A(packed,false,ELL);
    out.a_add.assign(add.data,add.data+first);
    out.b_add.assign(add.data+first,add.data+2*first);
    out.c_add.assign(add.data+2*first,add.data+3*first);
  });
  return out;
}
...
MuxTriple prepare_mux_triple(FPMath&math,IOPack&io,PRG128&rng,int party,int n){
  ...
  for(int i=0;i<n;++i){t.a_xor.data[i]=uint8_t(raw[i]&1);t.b.data[i]=raw[n+i]&MASK;}
  t.prep=measure(io,[&]{
...
FixArray mixed_beaver_mux(IOPack&io,int party,const BoolArray&sel,const FixArray&x,
                          const MuxTriple&t){
  const int n=x.size;
  if(sel.size!=n||t.a_xor.size!=n)throw std::runtime_error("mixed mux size");
  std::vector<uint8_t> emine(n),epeer(n);
  std::vector<uint64_t> dmine(n),dpeer(n);
  for(int i=0;i<n;++i){
    emine[i]=(sel.data[i]^t.a_xor.data[i])&1;
...
void print_phase(const char*name,int party,const Phase&p){
  std::cout<<"EXP217_PHASE party="<<party<<" name="<<name
...
int main(int argc,char**argv){
  ...
  auto bool_triples=prepare_bool_triples(math,io,ot,party,2*N);
  auto mux_align=prepare_mux_triple(math,io,rng,party,N);
  auto mux_merge=prepare_mux_triple(math,io,rng,party,N);
  ...
  auto ph_align=measure(io,[&]{
    ...
    for(int i=0;i<N;++i)aligned.data[i]=(negz.data[i]+pos_term.data[i])&MASK;
  });
  ...
  FixArray cert_add,marks_a(party,N,false,ELL,0);
  auto ph_cert=measure(io,[&]{
    ...
    beaver_bool_and_dual(io,party,lo,hi,
      ...
      cert,cert_add);
    safe_positive=beaver_bool_and(io,party,cert,pred_pos,
      bool_triples.a.data()+N,bool_triples.b.data()+N,bool_triples.c.data()+N);
  });

  // mark=1-cert is already available in additive sharing; no online B2A.
  uint64_t w_share=0;
  auto ph_count=measure(io,[&]{
    for(int i=0;i<N;++i){
      marks_a.data[i]=((party==ALICE?1ULL:0ULL)-cert_add.data[i])&MASK;
  ...
  if(accepted){
    ...
    ph_merge=measure(io,[&]{
      auto known=mixed_beaver_mux(io,party,safe_positive,z,mux_merge);
  ...
  print_phase("dual_share_mark_count",party,ph_count);print_phase("kangaroo_accept_and_dummy",party,ph_dummy);
  ...
  print_phase("kcompare_count_preprocess",party,k_count.phase);print_phase("bool_triple_preprocess",party,bool_triples.prep);print_phase("mixed_mux_align_preprocess",party,mux_align.prep);print_phase("mixed_mux_merge_preprocess",party,mux_merge.prep);
  print_phase("shuffle_setup",party,ph_shuffle_setup);print_phase("shuffle_preprocess",party,ph_shuffle_pre);

  std::cout<<"EXP217_RESULT party="<<party<<" N="<<N<<" capacity="<<CAP
