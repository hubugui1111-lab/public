...
void print_phase(const char* name,int party,const Phase&p){
  std::cout<<"EXP197_PHASE party="<<party<<" name="<<name<<" bytes="<<p.bytes<<" rounds="<<p.rounds<<" ms="<<p.ms<<"\n";
}
}
...
int main(int argc,char**argv){
  ...
  if(party==ALICE) for(int l=0;l<LEAVES;++l) for(int i=0;i<N;++i){
    bool pos=pred_bit(l,i)!=0;
    param_clear[l*ROW+i]=uint64_t(BOUND);
  ...
  FixArray bound(party,N,true,ELL,0),pred_pos_arith(party,N,true,ELL,0);
  for(int i=0;i<N;++i){
    uint64_t b=0,p=0;
    for(int l=0;l<LEAVES;++l){ b=(b+masked_params.data[l*ROW+i])&MASK; p=(p+masked_params.data[l*ROW+N+i])&MASK; }
    bound.data[i]=b; pred_pos_arith.data[i]=p;
  }

  // Clear value is exactly 0/1, so the LSB of additive shares is an XOR share
  // of the same bit. No interaction is needed for this A2B special case.
  BoolArray pred_pos(party,N);
  for(int i=0;i<N;++i) pred_pos.data[i]=uint8_t(pred_pos_arith.data[i]&1);
  ...
  print_phase("leaf_param_select",party,ph_leaf); print_phase("certificate_align",party,ph_align);
  ...
  std::cout<<"EXP197_RESULT party="<<party<<" N="<<N<<" capacity="<<CAP
