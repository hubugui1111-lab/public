...
#include "../exp196_orcompact/shared_controls.h"
#include "prepared_shared_leaf_lookup16_wide.h"
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdint>
...
constexpr int KELL=56; constexpr uint64_t KMASK=(1ULL<<KELL)-1, ZETA=(1ULL<<26);
...
struct PreparedRouteGC222{
  FPMath& math;
  OTPack& ot;
  SemiHonestParty<NetIO>* pe=nullptr;
  PreparedRouteGC222(FPMath&m,OTPack&o):math(m),ot(o){
  ...
  uint8_t run(const BoolArray&probe_sign,uint8_t leaf_mask){
    ...
    auto mux=[](const Bit&sel,const Bit&one,const Bit&zero){return (sel&(one^zero))^zero;};
    Bit b0=x[0];
    Bit b1=mux(b0,x[2],x[1]);
    Bit d20=mux(b1,x[4],x[3]),d21=mux(b1,x[6],x[5]);
    Bit b2=mux(b0,d21,d20);
    Bit d30=mux(b2,x[8],x[7]),d31=mux(b2,x[10],x[9]);
    Bit d32=mux(b2,x[12],x[11]),d33=mux(b2,x[14],x[13]);
    Bit d310=mux(b1,d31,d30),d311=mux(b1,d33,d32);
...
struct RealPolicy252{
  int layer=0,best_depth=0;
  std::array<uint16_t,INTERNAL> probes{};
  std::vector<uint64_t> bounds;
  std::vector<uint8_t> signs;
};
RealPolicy252 load_policy252(const std::string&path){
  std::ifstream f(path,std::ios::binary);
  if(!f) throw std::runtime_error("cannot open real policy");
  char magic[8];f.read(magic,8);
  if(std::string(magic,8)!="E252POL1")throw std::runtime_error("bad real policy magic");
  uint32_t layer=0,best=0;f.read(reinterpret_cast<char*>(&layer),4);f.read(reinterpret_cast<char*>(&best),4);
  RealPolicy252 p;p.layer=int(layer);p.best_depth=int(best);
  f.read(reinterpret_cast<char*>(p.probes.data()),INTERNAL*2);
  p.bounds.resize(size_t(LEAVES)*N);p.signs.resize(size_t(LEAVES)*N);
  f.read(reinterpret_cast<char*>(p.bounds.data()),std::streamsize(p.bounds.size()*8));
  f.read(reinterpret_cast<char*>(p.signs.data()),std::streamsize(p.signs.size()));
  if(!f)throw std::runtime_error("short real policy");
  return p;
}
std::vector<int64_t> load_z252(const std::string&path){
  std::ifstream f(path,std::ios::binary|std::ios::ate);
  if(!f)throw std::runtime_error("cannot open real z");
  auto n=f.tellg();f.seekg(0);
  if(n!=std::streamsize(N*sizeof(int64_t)))throw std::runtime_error("bad real z size");
  std::vector<int64_t> z(N);f.read(reinterpret_cast<char*>(z.data()),n);
  return z;
}
int64_t dec56(uint64_t x){
  x&=KMASK;
  return (x&(1ULL<<(KELL-1)))?int64_t(x)-int64_t(1ULL<<KELL):int64_t(x);
}
void share_alice_56(IOPack&io,int party,PRG128&rng,int n,
                    const std::vector<uint64_t>&clear,std::vector<uint64_t>&local){
  local.resize(n);std::vector<uint64_t>peer(n);
  if(party==ALICE){
    rng.random_data(local.data(),size_t(n)*8);
    for(int i=0;i<n;++i){local[i]&=KMASK;peer[i]=(clear[i]-local[i])&KMASK;}
...
struct PrivateGather252{
  ...
  Phase prep;
  PrivateGather252(int party,IOPack&io):rev16(3-party,io.io_rev,16),rev256(3-party,io.io_rev,256){
    rev16.set_precomp_batch_size(INTERNAL*256);
    rev256.set_precomp_batch_size(INTERNAL);
    prep=measure(io,[&]{rev16.preprocess();rev256.preprocess();});
  }
};
std::vector<uint64_t> private_gather252(IOPack&io,int party,PRG128&rng,PrivateGather252&gp,
                                        const std::array<uint16_t,INTERNAL>&idx,
                                        const std::vector<uint64_t>&local){
  constexpr int BLOCK=256,BLOCKS=12;
  std::vector<uint64_t>s1out(INTERNAL*BLOCK),r1(INTERNAL*BLOCK);
  std::vector<uint8_t>hi(INTERNAL*BLOCK);
  ...
  else gp.rev16.recv(s1out.data(),hi.data(),INTERNAL*BLOCK,64);
  std::vector<uint64_t>tq(INTERNAL),s2out(INTERNAL),out(INTERNAL);
  std::vector<uint8_t>lo(INTERNAL);
...
struct KComparePrep56{
  int n=0;bool used=false;
  std::vector<uint64_t>alpha,beta,a,b,c;
  std::vector<uint8_t>flip;
  Phase phase;
};
KComparePrep56 prepare_kcompare56(FPMath&math,IOPack&io,int party,PRG128&rng,int n){
  KComparePrep56 p;p.n=n;p.flip.assign(n,0);
  ...
  if(party==ALICE){
    ...
    for(int i=0;i<n;++i){
      uint64_t A0=2+(raw[3*i]%(ZETA-2));
      uint64_t B0=1+(raw[3*i+1]%(A0-1));
      uint8_t f=uint8_t(raw[3*i+2]&1);p.flip[i]=f;
      int64_t sg=f?-1:1;
      ac[i]=uint64_t(sg*int64_t(A0))&KMASK;
      bc[i]=uint64_t(sg*int64_t(B0))&KMASK;
    }
  }
  share_alice_56(io,party,rng,n,ac,p.alpha);
  share_alice_56(io,party,rng,n,bc,p.beta);
  p.a.resize(n);p.b.resize(n);rng.random_data(p.a.data(),size_t(n)*8);rng.random_data(p.b.data(),size_t(n)*8);
  for(int i=0;i<n;++i){p.a[i]&=KMASK;p.b[i]&=KMASK;}
  FixArray aa(party,n,false,KELL,0),bb(party,n,false,KELL,0);
  std::copy(p.a.begin(),p.a.end(),aa.data);std::copy(p.b.begin(),p.b.end(),bb.data);
  auto cc=math.fix->mul(aa,bb,KELL);flush_all(io);
  p.c.assign(cc.data,cc.data+n);
  p.phase={io.get_comm()-b0,io.get_rounds()-r0,std::chrono::duration<double,std::milli>(Clock::now()-t0).count()};
  return p;
}
BoolArray kangaroo_ge0_56(IOPack&io,int party,PRG128&rng,KComparePrep56&p,
                          const std::vector<uint64_t>&dshare){
  if(p.used||int(dshare.size())!=p.n)throw std::runtime_error("k56 invariant");
  p.used=true;const int n=p.n;
  std::vector<uint64_t>masked(size_t(2)*n),peer(size_t(2)*n),opened(size_t(2)*n);
  for(int i=0;i<n;++i){masked[i]=(p.alpha[i]-p.a[i])&KMASK;masked[n+i]=(dshare[i]-p.b[i])&KMASK;}
  if(party==ALICE){io.io->send_data(masked.data(),int(masked.size()*8));io.io->flush();io.io->recv_data(peer.data(),int(peer.size()*8));}
  else{io.io->recv_data(peer.data(),int(peer.size()*8));io.io->send_data(masked.data(),int(masked.size()*8));io.io->flush();}
  for(size_t i=0;i<opened.size();++i)opened[i]=(masked[i]+peer[i])&KMASK;
  ...
  for(int i=0;i<n;++i){
    ...
    tshare[i]=(uint64_t(v)+p.beta[i])&KMASK;
  }
  BoolArray result(party,n);
  if(party==ALICE){
    io.io->send_data(tshare.data(),n*8);io.io->flush();
    std::vector<uint8_t>mb(n);io.io->recv_data(mb.data(),n);
    for(int i=0;i<n;++i)result.data[i]=uint8_t(mb[i]^p.flip[i]);
  }else{
    std::vector<uint64_t>s(n);io.io->recv_data(s.data(),n*8);
    std::vector<uint8_t>mb(n),cs(n);std::vector<uint64_t>rr(n);rng.random_data(rr.data(),size_t(n)*8);
    for(int i=0;i<n;++i){
      uint8_t bit=uint8_t(dec56((tshare[i]+s[i])&KMASK)>=0);
      cs[i]=uint8_t(rr[i]&1);mb[i]=uint8_t(bit^cs[i]);result.data[i]=cs[i];
    }
    io.io->send_data(mb.data(),n);io.io->flush();
  }
  return result;
}
...
struct AdditiveSide227{
  int m=0;
  std::vector<uint32_t> perm,delta,recvA,recvB; // 3 ring words/row
};
AdditiveSide227 read_additive_side227(const std::string&path){
  std::ifstream f(path,std::ios::binary);
  if(!f)throw std::runtime_error("cannot open additive perm file");
  uint32_t magic=0,m=0;f.read((char*)&magic,4);f.read((char*)&m,4);
  if(magic!=0x41323237u||int(m)!=M)throw std::runtime_error("bad additive perm header");
  AdditiveSide227 d;d.m=int(m);d.perm.resize(m);d.delta.resize(size_t(m)*3);
  d.recvA.resize(size_t(m)*3);d.recvB.resize(size_t(m)*3);
  f.read((char*)d.perm.data(),size_t(m)*4);
  f.read((char*)d.delta.data(),size_t(m)*3*4);
  f.read((char*)d.recvA.data(),size_t(m)*3*4);
  f.read((char*)d.recvB.data(),size_t(m)*3*4);
  if(!f)throw std::runtime_error("short additive perm file");
  return d;
}
void install_owner227(PreparedPerm&p,const AdditiveSide227&d){
  p.p=d.perm;p.fwd.owner.delta.resize(size_t(M)*2);p.inv.owner.delta.resize(M);
  for(int i=0;i<M;++i){
    p.fwd.owner.delta[size_t(i)*2]=d.delta[size_t(i)*3];
    p.fwd.owner.delta[size_t(i)*2+1]=d.delta[size_t(i)*3+1];
    p.inv.owner.delta[i]=d.delta[size_t(i)*3+2];
  }
}
void install_recv227(PreparedPerm&p,const AdditiveSide227&d){
  p.p.resize(M);p.fwd.recv.A.resize(size_t(M)*2);p.fwd.recv.B.resize(size_t(M)*2);
  p.inv.recv.A.resize(M);p.inv.recv.B.resize(M);
  for(int i=0;i<M;++i){
    p.fwd.recv.A[size_t(i)*2]=d.recvA[size_t(i)*3];
    p.fwd.recv.A[size_t(i)*2+1]=d.recvA[size_t(i)*3+1];
    p.fwd.recv.B[size_t(i)*2]=d.recvB[size_t(i)*3];
    p.fwd.recv.B[size_t(i)*2+1]=d.recvB[size_t(i)*3+1];
    p.inv.recv.A[i]=d.recvA[size_t(i)*3+2];
    p.inv.recv.B[i]=d.recvB[size_t(i)*3+2];
  }
}
...
int main(int argc,char**argv){
  ...
  if(party==ALICE){
    ...
    for(int masked=0;masked<LEAVES;++masked){
      ...
      for(int i=0;i<N;++i){
        clear_bounds[size_t(masked)*N+i]=policy.bounds[size_t(real_leaf)*N+i]&KMASK;
        clear_signs[size_t(masked)*N+i]=policy.signs[size_t(real_leaf)*N+i]&1;
      }
    }
  }
  auto leaf_lookup=std::make_unique<exp252::PreparedSharedLeafLookup16Wide>(math,rng,N,KELL,clear_bounds,clear_signs);
  ...
  std::unique_ptr<PreparedRouteGC222> prepared_route;
  Phase ph_route_pre=measure(io,[&]{prepared_route=std::make_unique<PreparedRouteGC222>(math,ot);});

  // Exp227: correlations are generated by secure-join and converted to
  // additive mod-2^24 shares entirely before the online query starts.
  // The online apply_regular/apply_inverse code below is unchanged from Exp222.
  auto additive_side=read_additive_side227(permfile);
  PreparedPerm p0,p1;
  if(party==ALICE){install_owner227(p0,additive_side);install_recv227(p1,additive_side);}
  else{install_recv227(p0,additive_side);install_owner227(p1,additive_side);}
  ...
  if(party==ALICE){
    ...
    for(int i=0;i<N;++i){
      if(z_int[i]<=-(1LL<<(ELL-1))||z_int[i]>=(1LL<<(ELL-1)))throw std::runtime_error("real z exceeds movement ring");
      z_clear[i]=enc_signed(z_int[i]);
      z56_clear[i]=uint64_t(z_int[i])&KMASK;
    }
  ...
  exp252::LeafLookupResult lr;FixArray bound(party,N,false,KELL,0);BoolArray pred_pos(party,N);
  ...
  for(int i=0;i<N;++i){
    ...
    cert_d[i]=(z56_local[i]-one)&KMASK;                         // z >= 1
    cert_d[N+i]=(bound.data[i]-z56_local[i]-one)&KMASK;       // z <= U-1
    cert_d[2*N+i]=(uint64_t(0)-z56_local[i]-one)&KMASK;       // z <= -1
    cert_d[3*N+i]=(bound.data[i]+z56_local[i]-one)&KMASK;     // z >= -U+1
  }
  BoolArray cert,safe_positive;
  auto ph_cert=measure(io,[&]{
    auto cmp=kangaroo_ge0_56(io,party,rng,k_cert,cert_d);
  ...
  if(party==ALICE){
    ...
    for(int d=0;d<DEPTH;++d){
      int pi=int(policy.probes[heap]);
      int bit=(z_int[pi]>0)?0:1;
      leaf=(leaf<<1)|bit;heap=2*heap+1+bit;
    }
    expected_leaf=leaf;
  }
  for(int i=0;i<N;++i){
    ...
    if(party==ALICE){
      ...
      int64_t zc=z_int[i];
      bool exp=u>0 && ((pp&&zc>=1&&zc<int64_t(u)) || (!pp&&zc>-int64_t(u)&&zc<0));
      expected_cert+=exp;mismatches+=int(bool(pub_cert.data[i])!=exp);
    }
  }
  ...
  std::cout<<"EXP252_RESULT party="<<party<<" N="<<N<<" capacity="<<CAP
